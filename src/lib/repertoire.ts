...
import { isPUA, isValidCJKBasicChar, unicodeBlockRank } from "./utils";
...
export const dynamicAnalysis = (
  ...
  for (const char of guozi) {
    if (!isValidCJKBasicChar(char)) {
      console.log(`Warning: 国字常用列表中包含无效汉字: ${char}`);
    }
  }
  const rootSequence = getRootSequence(repertoire, config);
  const all = characters.map((x) => {
    ...
    return {
      ...
      gb2312: (repertoire[x]?.gb2312 ?? 0) > 0,
      国字常用: guozi.has(x),
      频率: frequency,
      读音: frequencies,
      拆分: segmentMap.get(x) ?? [],
  ...
  for (const [char, analysisList] of segmentDynamicAnalysis) {
    const current = analysisList.find((x) =>
      x.every((c) => config.roots.has(c) || /[0-9]/.test(c)),
    )!;
    if (current) {
  ...
  const 当前拆分: [string, string[]][] = [];
  for (const { 汉字, 拆分 } of all) {
    const strokeSequence = sequenceMap.get(汉字)!;
    const sequence = 拆分.flatMap((x) => currentDynamicAnalysis.get(x)!);
    const derived = sequence
      .map((x) => rootSequence.get(x)?.join("") ?? x)
      .join("")
      .replace(/6/g, "5");
    当前拆分.push([汉字, sequence]);
    if (strokeSequence !== derived) {
      mismatch.set(汉字, [strokeSequence, derived]);
    }
  }
  ...
  return {
    ...
    当前拆分: sortBy(当前拆分, ([a]) => unicodeBlockRank(a.codePointAt(0)!)),
