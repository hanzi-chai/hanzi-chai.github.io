...
export type 词典 = { 词: string; 拼音: string[] }[];
...
export function 解析频率映射(tsv: string[][]): 频率映射 {
  const data: 频率映射 = new Map();
  for (const [词, 拼音, 频率] of tsv) {
    if (词 === undefined || 拼音 === undefined || 频率 === undefined) continue;
    const maybeNumber = Number(频率);
    if (Number.isNaN(maybeNumber)) continue;
    data.set(`${词}:${拼音}`, maybeNumber);
  }
  return data;
}
...
export function 解析词典(tsv: string[][]): 词典 {
  const result: 词典 = [];
  for (const [word, pinyin_s] of tsv) {
    if (word === undefined || pinyin_s === undefined) continue;
    const pinyin = pinyin_s.split(" ");
    result.push({ 词: word, 拼音: pinyin });
  }
  return result;
}
