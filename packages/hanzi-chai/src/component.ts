...
import type { 安排, 安排描述, 条件, 退化配置 } from "./config.js";
import { 是变量, 是归并 } from "./config.js";
  ...
  生成二进制字根映射(字根部件列表: 部件[], 退化配置: 退化配置, 分类器: 分类器) {
    const 二进制字根映射 = new Map<number, 字根>(
      this.笔画列表.map((stroke, index) => {
        ...
        return [二进制数, 单笔.创建(分类器[stroke.feature])];
      }),
    );
...
class 默认部件分析器 extends 部件分析器<默认部件分析> {
  ...
  动态分析(部件: 部件) {
    ...
    const 自定义字根序列列表 = this.配置.动态自定义分析映射.get(部件);
...
class 冰雪飞花分析器 {
  ...
  public 字根名称映射 = new Map<string, 字根>();
  public 字根按键 = new Map<字根, string>();
  ...
  证明该字根只会出现在小集合(字根: 字根) {
    const 全部安排列表: 安排描述[] = [];
    全部安排列表.push(...(this.配置.字根决策空间.get(字根) ?? []));
    const 当前安排 = this.配置.字根决策.get(字根);
    if (当前安排)
    ...
    for (const { condition, value } of 全部安排列表) {
      if (typeof value === "string") {
        ...
        const 归并字根 = this.字根名称映射.get(value.element)!;
        const 归并限制 =
          condition?.some((x) =>
            isEqual(x, {
  ...
  当前在小集合(字根: 字根) {
    return this.小集合.test(this.字根按键.get(字根) ?? "");
  }
...
class 冰雪飞花部件分析器 extends 部件分析器<冰雪飞花部件分析> {
  ...
  constructor(private 配置: 字形分析配置) {
    ...
    console.log(
      "必要小集合字根",
      [
        ...new Set(this.分析器.字根按键.keys()).difference(
          this.分析器.可选字根与必要一般字根,
        ),
      ].map((x) => x.获取名称()),
    );
  }
  ...
  分析(部件: 部件) {
    ...
    const 全字根当前拆分方式 = rest.全部拆分方式.find((x) =>
      x.拆分方式.every((y) => this.分析器.字根按键.has(y.字根)),
    );
    if (!实际拆分方式 || !全字根当前拆分方式) {
      console.log(
        `部件「${部件.字符.toString()}」全部拆分方式：`,
        rest.全部拆分方式,
      );
  ...
  动态分析(部件: 部件) {
    ...
    for (const 拆分方式 of 可用拆分方式列表) {
      ...
      if (考虑全集合) {
        ...
        全集合分析列表.push(全集合分析);
        // 如果全集合拆分方式的字根都不是​可选字根，说明可以直接停止了
        if (全集合分析.条件列表.length === 0) 考虑全集合 = false;
      }
    }
