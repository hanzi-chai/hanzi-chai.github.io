...
import type { 基本部件分析, 字形分析配置, 字根 } from "./repertoire.js";
import {
  type 拆分方式,
  type 拆分环境,
  type 筛选器,
...
class 默认部件分析器 extends 部件分析器<默认部件分析> {
  ...
  分析(部件: 部件) {
    ...
    if (!分析.ok) return 分析;
    return ok(定制化分析(部件, 分析.value, this.配置));
  }
  ...
  动态分析(部件: 部件) {
    ...
    return ok(动态定制化分析(部件, 结果列表, this.配置));
  }
}
...
class 二笔部件分析器 extends 部件分析器<基本部件分析> {
  ...
  动态分析(部件: 部件) {
    ...
    if (!是必要字根) {
      const 前两笔和末笔 = this.前两笔和末笔(部件);
      if (!前两笔和末笔.ok) return 前两笔和末笔;
      结果列表.push({ 类型: "部件", 字根序列: 前两笔和末笔.value, 部件 });
    }
    return ok(结果列表);
  }
}
...
class 逸码部件分析器 extends 部件分析器<逸码部件分析> {
  ...
  分析(部件: 部件) {
    ...
    const 分析 = 定制化分析(部件, 可选分析.value, this.配置);
    let 余二拆分方式: 逸码拆分方式,
      余一拆分方式: 逸码拆分方式,
      笔画拆分方式: 逸码拆分方式;
