...
> {
  ...
  执行笔顺(
    ...
    for (const { index, from, to } of 笔顺) {
      ...
      if (部件分析 === undefined || !("当前拆分方式" in 部件分析)) {
        字根序列.push(...剩余部分字根序列);
        剩余字根序列列表[index] = [];
      } else {
...
> {
  ...
  首末取大顺序取根(字形: 字形, 末尾 = true): 字根[] {
    ...
    const 部分结果列表 = 字形.部分列表.map((子字形) => {
      ...
      const 当前是末尾 =
        排序部分列表.indexOf(子字形) === 排序部分列表.length - 1;
...
class 首右复合体分析器 extends 复合体分析器<默认部件分析> {
  ...
  分析(复合体: 复合体) {
    ...
    if (字根 !== undefined && this.配置.字根决策.has(字根)) {
      ...
      if (!第一部 || !第二部) return default_err(`${复合体.id} 缺少部分`);
    ...
    if (排序部分结果.length === 3) {
      ...
      if (第二部 instanceof 部件) {
        ...
        if (第二部字根 !== undefined && this.配置.字根决策.has(第二部字根)) {
          ...
          if (!第二部之一 || !第二部之二)
            return default_err(`${复合体.id} 缺少部分`);
  ...
  动态分析(复合体: 复合体) {
    ...
    if (排序部分结果.length === 3) {
      ...
      if (第二部 instanceof 部件) {
        ...
        if (第二部结果 === undefined)
          return default_err(`${第二部.id} 查询不到`);
        分析列表.push(
          ...贝叶斯推断([第一部结果, [...第二部结果]] as const, ([x, y]) => ({
            复合体,
            字根序列: [x.字根, ...y.字根序列],
...
> {
  ...
  取全集合首根和余部(字形: 字形): 冰雪飞花部分信息 {
    ...
    if (部件分析结果 === undefined)
      throw new Error(`部件 ${字形.id} 没有分析结果`);
    return {
      部分: 字形,
  ...
  组合逻辑(
    ...
    const 是准包围结构 = [0x1].includes(首部信息.部分.id);
    ...
    if (是包围结构 || 是准包围结构) {
      ...
      if (首部信息.是单字根 && 末部信息.是单字根) {
        ...
        部首取法 = 特取字.includes(复合体.id.toString()) ? "末" : "首";
      } else if (首部信息.是单字根) {
        部首取法 = "首";
      } else if (末部信息.是单字根) {
        部首取法 = "末";
