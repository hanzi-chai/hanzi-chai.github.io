...
class 原始字库 {
  ...
  确定(
    ...
    // 最后过滤一次
    const 当前标签集合 = new Set(字形来源列表);
    const 过滤后确定字库 = new 字库();
    for (const { 字符, 字形列表 } of 确定字库) {
  ...
  替换(
    ...
    for (const 部分 of 项.operandList) {
      const result = this.替换(字形映射, 部分, 变量映射);
      if (!result.ok) return result;
      if (result.value instanceof 字符) {
        ...
        字形映射.set(如码位.value, [result.value]);
   ...
   * 应用变换器到数据库，返回新的数据库。
   */
  应用变换器(字形映射: Map<字符, 标签字形数据[]>, 变换器: 变换器) {
    const 新映射 = new Map<字符, 标签字形数据[]>();
    for (const [码位, 字形列表] of 字形映射) {
      const 新列表: 标签字形数据[] = [];
      for (const 字形 of 字形列表) {
        const 变量映射: 变量映射 = new Map();
        if (this.模式匹配(字形映射, 字形, 变换器.from, 变量映射)) {
          const 新字形 = this.替换(字形映射, 变换器.to, 变量映射);
