...
class 原始字库 {
  ...
  确定(
    自定义字形: 字形自定义 = {},
    标签列表 = 所有地区标签 as readonly string[],
  ): Result<字库, Error> {
    const 确定字库 = new 字库();
    const 字形列表映射 = new Map<string, 字形数据[]>();
    ...
    for (const [汉字名, 汉字] of Object.entries(this.字库)) {
      ...
      if (是用户私用区(汉字名)) {
        ...
        if (!字形列表.some((x) => 获取地区标签列表(x).length === 0)) {
          ...
          for (const glyph of glyphs) {
            const 剩余地区标签列表 = 获取地区标签列表(glyph).filter(
              (x) => !已有地区标签集合.has(x) && 标签列表.includes(x),
            );
        ...
        if (字形列表.length === 0) {
          // 如果没有任何字形被选中，那么就使用所有字形（不考虑地区标签）
          字形列表.push(glyphs[0] ?? 模拟基本部件());
        }
      }
      字形列表映射.set(汉字名, 字形列表);
    }
    for (const [汉字名, 字形列表] of 字形列表映射.entries()) {
      const 汉字 = this.字库[汉字名]!;
      const { ambiguous: _, ...rest } = 汉字;
      const 渲染后字形列表: 约化字形数据[] = [];
      for (const glyph of 字形列表) {
        const 渲染后字形 = this.递归渲染原始字形(
          glyph,
          字形列表映射,
          字形缓存,
          [汉字名],
  ...
  获取源部件(
    字符: string,
    字形列表映射: Map<string, 字形数据[]>,
    字形缓存: Map<string, 矢量图形数据> = new Map(),
    栈: string[] = [],
  ): Result<基本部件数据, Error> {
    const 字形列表 = 字形列表映射.get(字符);
    const 字符信息 = `${字符} (U+${码(字符)} )`;
    const 栈信息 = 栈.map((x) => `${x} (U+${码(x)})`).join(" -> ");
    if (字形列表 === undefined)
      return default_err(`源部件 ${字符信息} 不存在，当前栈：${栈信息}`);
    let 源字形 = 字形列表.find(是部件或全等);
    if (源字形 === undefined) {
      console.warn(
        `源部件 ${字符信息} 没有部件或全等，当前栈：${栈信息}，将使用原始字库中的数据代替。`,
      );
      源字形 = this.字库[字符]?.glyphs?.find(是部件或全等);
      if (源字形 === undefined)
        return default_err(
          `源部件 ${字符信息} 没有部件或全等，当前栈：${栈信息}`,
        );
    }
    const 渲染后源字形 = this.递归渲染原始字形(源字形, 字形列表映射, 字形缓存, [
      ...栈,
      字符,
    ]);
  ...
  递归渲染原始字形(
    ...
    if (字形.type === "derived_component") {
      ...
      for (const 部分 of 字形.operandList) {
        const 渲染后源字形 = this.获取源部件(部分, 字形列表映射, 字形缓存, 栈);
        if (!渲染后源字形.ok) return 渲染后源字形;
      ...
      const 引用字形 = 字形列表映射.get(字形.source)?.[0];
      if (引用字形 === undefined)
        return default_err(
          `源部件 ${字形.source} 不存在，当前栈：${栈.join(" -> ")}`,
        );
      return this.递归渲染原始字形(引用字形, 字形列表映射, 字形缓存, [
        ...栈,
        字形.source,
      ]);
    } else {
