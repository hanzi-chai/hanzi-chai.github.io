...
export const bias: Sieve = (_, scheme) => scheme.map(countStrokes).map(x => -x);
...
const select = (sieveList: Sieve[], name: string, component: Glyph, schemeList: Scheme[], rootMap: Map<number, string>) => {
  ...
  schemeList.forEach(v => {
    schemeData.set(v.toString(), ({ key: v.toString(), roots: v.map(lookup), attaching: -1, crossing: -1 }))
  });
  for (const sieve of sieveList) {
    ...
    currentSchemeList = currentSchemeList.filter((_, index) => scoreList[index] === min);
  }
  if (currentSchemeList.length === 1) {
    return { best: currentSchemeList[0].map(lookup), schemes: [...schemeData.values()] } as ComponentResult;
  }
